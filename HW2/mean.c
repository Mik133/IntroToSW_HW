...
void print_mean(int all_grades[],int all_grades_size);
...
int main(int argc,char *argv[])
{
	...
	print_mean(all_grades,all_grades_size);
	free(all_grades);
	free(grade);
	fclose(grades_file);
	return 0;
...
{
	double grades_sum = 0;
	for(int i = 0;i < all_grades_size;i++)
		grades_sum += all_grades[i];
	printf("%.2f",(double)grades_sum/(double)all_grades_size);
}
